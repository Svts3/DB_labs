...
DELIMITER //
CREATE TRIGGER delete_continent_trigger BEFORE delete ON continent FOR EACH ROW 
BEGIN
	delete from country where continent_name = old.name;
end //
DELIMITER //
CREATE TRIGGER create_continent_trigger BEFORE INSERT ON continent FOR EACH ROW 
BEGIN
...
Delimiter //
-- 3(E)
CREATE TRIGGER log_continent_deletion BEFORE Delete ON continent FOR EACH ROW
begin
		insert into continent_delete_logger(continent, deletion_time) values(old.name, now());
end //
Delimiter //
-- 3(J)
CREATE TRIGGER OWNER_FIRST_NAME_INSERTION_VALIDATION BEFORE INSERT ON user FOR EACH ROW
begin
	IF(not(new.first_name = "Svitlana" or new.first_name = "Olha" or new.first_name = "Taras" or new.first_name = "Petro"))then
